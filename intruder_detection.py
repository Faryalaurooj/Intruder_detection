...
parser = argparse.ArgumentParser(description="Intruder detection with YOLOv8")
...
# Open the video file
cap = cv2.VideoCapture(args.video_path)  # Use the video path from the argument
...
roi = cv2.selectROI("Intruder detection with YOLOv8", frame, fromCenter=False, showCrosshair=True)
...
while cap.isOpened():
    ...
    out.write(frame)
    cv2.imshow("Intruder detection with YOLOv8", frame)
